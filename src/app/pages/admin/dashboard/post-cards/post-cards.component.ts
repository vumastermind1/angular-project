...
@Component({
  selector: 'flikshop-post-cards',
  templateUrl: './post-cards.component.html',
  styleUrls: ['./post-cards.component.scss']
})
export class PostCardsComponent extends BaseAnalyticsComponent implements OnInit, OnChanges {

  @Input() duration: Moment[];
  @ViewChild('postcardsChart', { static: true }) postcardsChart: ElementRef;
  ...
  async drawChart(start: string, end?: string, expanded?: boolean) {
    ...
    const targetElement = this.expanded ? this.expandedChart.nativeElement : this.postcardsChart.nativeElement;
    ...
    const rawData: any = await this.analyticsService.getPostcardsShipped(startDate, endDate).toPromise();
    const data = {
      labels: [],
      series: []
    };
  ...
  createTooltip(value: number, parentElement: HTMLElement) {
    super.createTooltip(value, parentElement, `Flikshops: ${value}`);
  }
}
